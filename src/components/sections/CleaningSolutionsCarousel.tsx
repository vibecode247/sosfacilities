  ...
  return <section className="py-16 md:py-20 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-4 md:mb-6 gradient-text font-outfit">Our Expertise</h2>
        ...
        <div className="relative max-w-7xl mx-auto" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
                ...
                {getCurrentSlideItems().map((solution, index) => <Card key={`${currentSlide}-${index}`} className="group hover:shadow-xl transition-all duration-300 border-0 neomorphism-card overflow-hidden cursor-pointer">
                    <div className="relative aspect-square overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex flex-col justify-end p-3 md:p-4 lg:p-6 z-10">
                        <div className="bg-white/15 backdrop-blur-md rounded-lg md:rounded-xl px-3 md:px-4 py-0 border border-white/20 group-hover:bg-white/20 transition-all duration-300">
          ...
          <div className="flex justify-center mt-6 md:mt-8 space-x-2 md:space-x-3">
            {Array.from({
            length: totalSlides
          }).map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 rounded-full ${currentSlide === index ? 'bg-primary-500 w-8 md:w-10 h-3 md:h-4 shadow-md' : 'bg-gray-300 hover:bg-gray-400 w-3 md:w-4 h-3 md:h-4'}`} aria-label={`Go to slide ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </section>;
